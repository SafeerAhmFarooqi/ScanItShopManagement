                ...
                <table class="table table-flush align-middle table-row-bordered table-row-solid gy-4 gs-9">
                    ...
                    <thead class="border-gray-200 fs-5 fw-bold bg-lighten">
                        <tr>
                            ...
                            <th class="min-w-150px">Shop Number</th>
                            <th class="min-w-150px">Due Amount</th>
                            <th class="min-w-150px">Initial Advance Payment</th>
                            <th class="min-w-150px">Advance Payment Left</th>
                            <th class="min-w-150px">Cut Off</th>
                            <th class="min-w-150px">Rent Start Date</th>
                            <th class="min-w-150px">Total Payments Amount</th>
                            <th class="min-w-150px">Total Months</th>
                            <th class="min-w-150px">Next Due Amount Date</th>
                        </tr>
                    </thead>
                    <!--end::Thead-->
                    <!--begin::Tbody-->
                    <tbody class="fw-6 fw-bold text-gray-600">
                        ...
                        <tr>
                            ...
                            <td>{{$rentalCustomer->floor->name}}</td>
                            <td>{{$rentalCustomer->shop->shopno}}</td>
                            <td>{{$this->getDueAmount($rentalCustomer->id)}}</td>
                            <td>{{$rentalCustomer->advancepayment}}</td>
                            <td>{{$this->getLeftAdvancePayment($rentalCustomer->id)}}</td>
                            <td>{{$rentalCustomer->cutoffpayment}}</td>
                            <td>{{$rentalCustomer->created_at->format('F d,y')}}</td>
                            <td>{{$this->getTotalPaymentsAmount($rentalCustomer->id)}}</td>
                            <td>{{$rentalCustomer->created_at->diffInMonths(\Carbon\Carbon::now())}}</td>
                            <td>{{($rentalCustomer->created_at->addMonths(($rentalCustomer->created_at->diffInMonths(\Carbon\Carbon::now()))+1))->format('F d,y')}}</td>
                        </tr>    
                        @endforeach    
                        @else
                            <tr>
